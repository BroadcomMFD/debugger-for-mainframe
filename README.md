...
- Java version 8.0 or higher installed on your client workstation with the PATH variable correctly configured.
...
- At least 50 MB of disk space in your USS home directory.
</details>
...
After you import your certificate, run a test debug session with **"interTestSecure": true** in your `launch.json` file. If the session fails, ensure that you imported the correct certificate to the correct JRE trust store and try again.
...
Debugger for Mainframe includes two VS Code walkthroughs that guide you through the basic COBOL demo sessions, which are shipped with InterTest for CICS and InterTest Batch. To access the walkthroughs, select **Welcome** from the **Help** menu, click **More...** under **Walkthroughs**, and select either **Basic CICS Demo Session** or **Basic Batch Demo Session**.

For the instructions on how to run the debugging demo sessions, see the "Debugging Demo Sessions" page in the [Code4z documentation](https://techdocs.broadcom.com/code4z).
...
5. Fill in the necessary fields as described in the **[Configure a Debug Session](#configure-a-debug-session)** section.

6. Run the debug session as described in the **[Run a Debug Session](#run-a-debug-session)** section.

You ran your first debug session.
</details>

<details>
<summary id="configure-a-debug-session"><span style="font-size: 1.5em"><b>Configure a Debug Session</b></span><hr></summary>

To debug CICS and Batch programs with Debugger for Mainframe, you open the workspace in your IDE and configure your connection to InterTest using the file `launch.json`. To debug Batch programs, you also convert the JCL of your program into a new file which is used for debugging.
...
<details>
<summary id="basic-cics-configuration"><span style="font-size: 1.3em">Basic CICS Configuration</span></summary>
...
To add a basic configuration to debug a CICS application, add a **Debugger for Mainframe: INTERTEST™ FOR CICS** configuration and populate the following fields:
  ...
  - **"apimlProfile"**: (string)
    - (Zowe API ML only) Specifies the name of a Zowe Explorer zosmf profile that contains mainframe credentials and the host and port of your Zowe API ML Gateway. Specify this field to use the Zowe API ML Single Sign-On feature.
  - **"path"**: (string)
  ...
  - **"paragraphBreakpoints"**: (boolean)
    - (Optional) Specify "true" to have the debugging session stop automatically at each new paragraph or label. For more information on paragraph breakpoints, see **[Breakpoints](#breakpoints)**.
  - **"callTrace"**: (boolean)
    - (Optional) Specify "false" to disable the call trace feature. The feature is enabled by default. For more information on call trace, see **[Tracing](#tracing)**.
  - **"statementTrace"**: (boolean)
    - (Optional) Specify "false" to disable the statement trace feature. The feature is enabled by default. For more information on statement trace, see **[Tracing](#tracing)**.
...
<details>
<summary id="basic-batch-configuration"><span style="font-size: 1.3em">Basic Batch Configuration</span></summary>
...
To add a basic configuration to debug a batch application, add a **Debugger for Mainframe: INTERTEST™ BATCH** configuration and populate the following fields:
  ...
  - **"executionCounts"**: (boolean)
    - (Optional) Specify "true" to enable the execution counts feature. The feature is disabled by default. For more information on execution counts, see **[Breakpoints](#breakpoints)**.
  - **"variableOrder"**: (string)
    - (Optional) Specify "alphabetical" to sort the variables that are defined in your program in alphabetical order in the variables view. Specify "definition" to sort the variables in the order of their definition in the program. The default setting is "definition".
...
<details>
<summary id="enable-the-batch-link-queue"><span style="font-size: 1.3em">Enable the Batch Link Queue</span></summary>
...
To enable the Batch Link Queue, add a **Debugger for Mainframe: INTERTEST™ FOR BATCH - Attach** configuration to your existing `launch.json` file, which must already contain at least one `intertest-batch` configuration. Populate the following fields:
  ...
  - **"interTestCharset"**: (string)
    - (Optional) Specifies the Testing Tools Server Charset for Listings. Specify this field only if your Testing Tools Server instance is configured to use a client code page other than UTF-8.
  - **"paragraphBreakpoints"**: (boolean)
    - (Optional) Specify "true" to have the debugging session stop automatically at each new paragraph or label. For more information, see **[Breakpoints](#breakpoints)**.
  - **"statementTrace"**: (boolean)
    - (Optional) Specify "false" to disable the statement trace feature. The feature is enabled by default. For more information, see **[Tracing](#tracing)**.
  - **"executionCounts"**: (boolean)
    - (Optional) Specify "true" to enable the execution counts feature. The feature is disabled by default. For more information, see **[Breakpoints](#breakpoints)**.
  - **"variableOrder"**: (string)
    - (Optional) Specify "alphabetical" to sort the variables that are defined in your program in alphabetical order in the variables view. Specify "definition" to sort the variables in the order of their definition in the program. The default setting is "definition".
</details>
...
- **"integrationMode"**: (string)
  - Specifies which protocol to use to communicate with InterTest. Specify **"CCI"** or **"TCP"**. The default is **"TCP"**.
- **"blsPort"**: (integer)
  - (TCP only) Specify the port number for the Java instance on USS to use to connect to InterTest. Specify either this parameter or "blsPortRange".
- **"blsPortRange"**: (JSON)
  - (TCP only) Specify a range of port numbers for the Java instance on USS to use to connect to InterTest. Specify either this parameter or "blsPort".
  - This is a JSON element that consists of the following parameters: 
    - **"start"**: (integer)
      - Specifies a minimum port number.
    - **"end"**: (integer)
      - Specifies a maximum port number.
- **"cciFamilyID"**: (string)
  - (Optional, CCI only) Specifies a unique CCS CCI ID for the InterTest component that is executed on mainframe over SSH. If you do not specify this parameter, the default value **"INTTSTSR"** is used.
- **"ITHLQ"**: (string)
  - Specifies the DSN prefix of the InterTest LPAR. 
- **"ITProfileDS"**: (string)
  - Specifies the InterTest PROFLIB data set name.
- **"ussJavaPath"**: (string)
  - (Optional) Specifies the USS path to a Java installation on the mainframe. If you do not specify this parameter, the Java installation in the PATH environment variable is used.
- **"ussWorkingDirectory"**: (string)
  - (Optional) Specifies a folder in your USS home directory to store executable components and temporary files for the extension. If you do not specify this parameter, the subfolder **/.intertest** is created in your USS home directory and used for this purpose.
- **"sshPort"**: (integer)
  - (Optional) Specifies the port used by the remote OpenSSH server. If you do not specify this parameter, the default port number **22** is used.
- **"ITStepLib"**: (string)
  - Specifies additional libraries that are required by the program that you want to debug. This parameter is mandatory for CICS sessions. Specify a string of data set names separated by colons.
 
To specify these parameters as environment variables in USS, use the same variable names as in the JSON element above.

</details>
<details>
<summary id="dynamic-symbolic-support"><span style="font-size: 1.3em">Dynamic Symbolic Support</span></summary>
...
<details>
<summary id="run-a-debug-session"><span style="font-size: 1.5em"><b>Run a Debug Session</b></span><hr></summary>

After you define your configuration in `launch.json`, you can run your debug session in the debugging interface.

<details>
<summary id="run-a-cics-or-batch-debugging-session"><span style="font-size: 1.3em">Run a CICS or Batch Debugging Session</span></summary>

Use the following procedure to run a CICS or Batch debugging session.
...
5. If more than one listing is found in the CICS regions or PROTSYMs specified in your configuration, or if no exact matching listing is found, a list of all available listings displays. Select the listing that you want to load.
...
<details>
<summary id="run-a-debug-session-from-the-batch-link-queue"><span style="font-size: 1.3em">Run a Debug Session from the Batch Link Queue</span></summary>

To run a debug session from the Batch Link Queue, ensure that you have an `attach` configuration in your `launch.json` file. For more information, see **[Configure a Debug Session](#configure-a-debug-session)**.

1. On the **Run and Debug** panel, select an `attach` configuration from the list of configurations.
...
<details>
<summary id="manage-the-batch-link-scheduling-table"><span style="font-size: 1.3em">Manage the Batch Link Scheduling Table</span></summary>

You can use Debugger for Mainframe to manage the Batch Link Scheduling Table, which enables you to debug Db2 Stored Procedures and IMS DC applications.
...
1. Press **F1** to open the Command Palette and select **Batch: Show Scheduling Table**.
...
The stored procedure is scheduled. To view it in the Batch Link Scheduling Table, repeat steps 1 to 3.
...
<details>
<summary id="breakpoints"><span style="font-size: 1.5em"><b>Breakpoints</b></span><hr></summary>
...
<details>
<summary id="conditional-and-unconditional-breakpoints"><span style="font-size: 1.3em">Conditional and Unconditional Breakpoints</span></summary>
...
Supported literals are as follows:
...
<details>
<summary id="data-breakpoints"><span style="font-size: 1.3em">Data Breakpoints</span></summary>
...
<details>
<summary id="paragraph-breakpoints"><span style="font-size: 1.3em">Paragraph Breakpoints</span></summary>

Set the `launch.json` parameter **paragraphBreakpoints** to "true" to trigger a breakpoint at the beginning of every new paragraph in COBOL code, and every label in Assembler code. For more information, see **[Configure a Debug Session](#configure-a-debug-session)**.

You can also turn this feature on and off in the debugger console by submitting the commands `/AT LABEL` and `/LABEL OFF`.
   </details>

<details>
<summary id="logpoints"><span style="font-size: 1.3em">Logpoints</span></summary>

![Animated gif showing a logpoint being set and triggered](https://raw.githubusercontent.com/BroadcomMFD/debugger-for-mainframe/master/LogPoints.gif "Animated gif of a logpoint being set and triggered")
...
<details>
<summary id="execution-counts"><span style="font-size: 1.3em">Execution Counts</span></summary>
...
The execution count feature is disabled by default. It can be enabled in `launch.json` before you begin your debug session. For more information, see **[Configure a Debug Session](#configure-a-debug-session)**.
...
<details>
<summary id="variables"><span style="font-size: 1.5em"><b>Variables</b></span><hr></summary>
...
COBOL table variables defined with the OCCURS X TIMES keyword can only be updated from the variables tree view, not from the editor.

<details>
<summary id="variables-tree-view"><span style="font-size: 1.3em">Variables Tree View</span></summary>
...
<details>
<summary id="filter-variables"><span style="font-size: 1.3em">Filter Variables</span></summary>
...
<details>
<summary id="edit-the-hexadecimal-value-of-a-variable"><span style="font-size: 1.3em">Edit the Hexadecimal Value of a Variable</span></summary>
...
<details>
<summary id="enable-inline-variables-view"><span style="font-size: 1.3em">Enable Inline Variables View</span></summary>

Enable inline variables view to see the values of all variables on the highlighted line of code inline.

![Inline variables view](https://raw.githubusercontent.com/BroadcomMFD/debugger-for-mainframe/master/inlinevars.png "Inline variables view")
...
<details>
<summary id="tracing"><span style="font-size: 1.5em"><b>Tracing</b></span><hr></summary>
...
The call trace and statement trace features are enabled by default. They can be disabled in `launch.json` before you begin your debug session. For more information, see **[Configure a Debug Session](#configure-a-debug-session)**.
...
To enable job status messages, the **zoweProfileName** parameter must be specified in your `launch.json` file. For more information, see **[Configure a Debug Session](#configure-a-debug-session)**.

Zowe Explorer is available as part of the Code4z Foundation pack.
...
<a href="https://www.openmainframeproject.org/all-projects/zowe/conformance"><img alt="This extension is Zowe v3 conformant" src="https://artwork.openmainframeproject.org/other/zowe-conformant/zowev3/explorer-vs-code/color/zowe-conformant-zowev3-explorer-vs-code-color.png" width=20% height=20% /></a>
   ...
   <details>
<summary id="troubleshooting"><span style="font-size: 1.5em"><b>Troubleshooting</b></span><hr></summary>
...
- **"logLevel"**:
    - Specifies the amount of information reported to the log. Specify one of the following values:
        ...
        - "FINEST"
        - "ALL"
    These values are ordered from returning the least information ("SEVERE"; errors only) to the most information ("ALL"; all details).
- **"logOutput"**:
...
<details>
<summary id="reportissues"><span style="font-size: 1.5em"><b>Report Issues</b></span><hr></summary>
